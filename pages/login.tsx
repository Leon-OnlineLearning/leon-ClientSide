...
export default function LoginPage() {
  ...
  const onGoogleSignInSuccess = (response: GoogleLoginResponse) => {
    axios
      .post(
        `${config.serverBaseUrl}/auth/google`,
      ...
      .then((_) => {
        handleRedirection(localStorage.getItem("role"));
      })
    ...
    axios
      ...
      .catch((err) => {
        ...
        if (!response || response.status == 500) {
          ...
          setErrorMsg("Login attempts limited has been exceeded");
        }
      });
  };
              ...
              <Form style={{ display: "flex", flexDirection: "column" }}>
                ...
                <div className={`${styles["login-group"]}`}>
                  <Button
                    type="submit"
                    onClick={handleLogin}
                    className={`my-2 ${styles["loginbtn"]}`}
                    variant="outline-primary"
                  >
                    <i className="bi bi-file-person-fill"></i> Login
                  </Button>{" "}
                  <GoogleLogin
                    clientId="627445614367-q6p0r94ibunlplu7mcoqi893fgmggeno.apps.googleusercontent.com"
                    theme="dark"
